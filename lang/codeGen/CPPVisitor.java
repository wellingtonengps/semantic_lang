...
public class CPPVisitor extends Visitor {
    ...
    private List<ST> funcPrototypes; // Add this list
    ...
    Env<LocalAmbiente<SType>> env;    // Ambiente do código
    ...
    public CPPVisitor(String fileName, Env<LocalAmbiente<SType>> env, HashMap<String, DataAttributes> datasAttrib) {
        groupTemplate = new STGroupFile("lang/codeGen/templates/c++.stg");
        ...
        if (f.getReturnTypes().size() < 2) {
            ...
            if (f.getParameters() != null) {
                ...
                for (int i = 0; i < paramsList.size(); i++) {
                    SType t = ((STypeFun) local.getFuncType()).getTypes()[i]; // Pega o tipo do parametro
            ...
            if(!f.getId().equals("main")){ // Não quero que seja criado o prototipo da função main
                funcPrototype = groupTemplate.getInstanceOf("funcPrototype");

                if(f.getReturnTypes().size() == 1){
                    funcPrototype.add("name", f.getId() + "_retorno_00");
                }

                if(f.getReturnTypes().size() == 0){
                    funcPrototype.add("name", f.getId());
                }

                //funcPrototype.add("name", f.getId());
                //funcPrototype.add("type", fun.getAttribute("type"));
                funcPrototype.add("type", fun.getAttribute("type"));
            ...
            for (int j = 0; j < f.getReturnTypes().size(); j++) {
                ...
                if(!f.getId().equals("main")){ // Não quero que seja criado o prototipo da função main
                    funcPrototype = groupTemplate.getInstanceOf("funcPrototype");
                    funcPrototype.add("name", f.getId() + "_retorno_0" + idRetorno); // nome original da função
                    funcPrototype.add("type", fun.getAttribute("type"));
                    funcPrototype.add("params", params);
                    funcPrototypes.add(funcPrototype);
                }
    ...
    private void processSType(SType t) {
        if (t instanceof STypeInt)
            type = groupTemplate.getInstanceOf("int_type");
        else if (t instanceof STypeBoolean)
            type = groupTemplate.getInstanceOf("boolean_type");
